""" [Gold 1]
1. 뱀의 머리를 1초당 한 칸씩 time초 만큼 전진한다.
2. 이동 전의 머리 위치부터 이동 후의 머리 위치를 양 끝점으로 갖는 선분으로 저장한다.
3. 위에서 구한 선분이 맵의 영역에서 벗어나거나 뱀이 이동했던 영역을 교차하지 않는다면
   파라미터를 조정하고 다음 명령을 수행한다.
4. 뱀의 머리가 영역에서 벗어나거나 뱀이 이동했던 영역을 교차한다면 이동거리를 반환하고 종료한다.
5. 모든 명령을 수행하고도 종료되지 않는다면 뱀의 머리가 마지막으로 향했던 방향으로 종료될 때까지 이동시킨다.
** L: 반시계, R: 시계방향으로 회전
"""
import sys
